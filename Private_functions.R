...
.Schwein<-function(ser.s,ser.j,ser.z,ser.v, sub){
  Schwein_ind<- c(sub[,1]/sub[,2], sub[,3]/sub[,4], sub[,2]/sub[,1], sub[,4]/sub[,3]) # Calculates excentrecity index and assignes it to the table
  return(Schwein_ind)
}
...
.seriesTRW_one<-function(t,plot=1,tree=1,level=1){
  ID<-.IDdistinct(t)
  ser.s <- as.character(ID$IDAspect[level,"S.OC"]) 
  ser.j <- as.character(ID$IDAspect[level,"J.OC"])
  ser.v <- as.character(ID$IDAspect[level,"V.OC"])
  ser.z <- as.character(ID$IDAspect[level,"Z.OC"])
  w <- t[,c(ser.s, ser.j, ser.v, ser.z)]
  return(w)
}# returns TRW for selected tree including NA values
.seriesTRW_two<-function(t,plot=1,tree=1,level=1){
  ID<-.IDdistinct(t)
  ser.s <- as.character(ID$IDAspect[level,"S.OC"]) 
  ser.j <- as.character(ID$IDAspect[level,"J.OC"])
  ser.v <- as.character(ID$IDAspect[level,"V.OC"])
...
.calcSumTRW<-function(serie,lengths){
  trws<-c()
  for(i in 0:((length(serie)))){
    a<-i+1
    l<-lengths[a]
    for(j in 1:((length(serie))+1)){
      b<-i*7+j
      if(lengths[b]>0){
        e<-sum(serie[1:l,j], na.rm = T)
      }else{
        e<-0
      }
      trws<-c(trws,e);
...
.replace<-function(repl){
  ...
  if(NAS>0){a<-NAS+1;b<-NAS+6;r[1:NAS,2]<-mean(repl[a:b,1])}
  if(NAE>0){a<-NAE+1;b<-NAE+6;r[1:NAE,3]<-mean(repl[a:b,1])}
  if(NAW>0){a<-NAW+1;b<-NAW+6;r[1:NAW,4]<-mean(repl[a:b,1])}
  return(r)
...
.elipseFun <- function(sirky=widths[1,], npoints=100){
  ...
  return(vystup)
}# výpoèet parametrù elipsy - jednotlivé body
...
.seriesCalc<-function(serie,heights){
  ...
  for(i in 0:(length(serie))){
    ...
    x<-length2[j]
    if(x==0){
      y<-max(lab)-max(lengths)
    }else{
      y<-lab[x]
    }
    z<-rep(y,times=length(lengths))
    year<-c(year,z)
  }
...
.dataGraphAlometry<-function(trw,vysky,plot=1,tree=1,dir="N-S"){
  ...
  tree<-subset(.IDdistinct(trw)$IDAspect,IDPlot==1 & IDTree==1)
  ...
  l<-.seriesCalc(left,vysky);r<-.seriesCalc(right,vysky)
...
} #Vypoète vzdálenost od osy y, pro západ a jih (levá cást grafu) se musí vynásobit -1
...
.calcSerieRight<-function(serie,vyska){
  ...
  return(linie)
} #pro kombinovaný graf vypoète vzdálenosti na ose X a Y pro pravou stranu
.calcSerieLeft<-function(serie,vyska){
  ...
  linie$hodnoty<-linie$hodnoty+(vyska-(max(linie$hodnoty)-min(linie$hodnoty))/2)
  return(linie)
} #pro kombinovaný graf vypoète vzdálenosti na ose X a Y pro levou stranu

#sada of drawing functions
.linie<-function(graf, vyska, dataLeft, dataRight){
  ...
  return(graf)
} #vykreslí jednu linii dle parametrù

.plotGraph<-function(tree,plot,heights,trw,extc,direction="North-South"){
  left<-0
  right<-0
  meta<-.treeSelect("1","1",extc)
  if(direction!="North-South"){left<-meta$Z.OC;right<-meta$V.OC}
  else{left<-meta$S.OC;right<-meta$J.OC}
  serieLeft<-.dataLine(extc,left) #extc
  serieRight<-.dataLine(extc,right)
  ...
  graph<-graph+scale_x_continuous(name="Stem width (mm)",limits=c(-150,150),breaks=seq(-150,150,50),labels=c(seq(75,0,-25),seq(0,75,25)))
  graph<-graph+scale_y_continuous(name="Stem height (cm)")
  return(graph)
}